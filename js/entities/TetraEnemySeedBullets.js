/**
 * TetraEnemySeedBullets.js
 * The normal Tetra enemy's phase-1 ranged attack (see TetraEnemy.js) — a
 * slow bullet fired from each of the hull's 4 sides while it bounces, that
 * detonates into a ring of shrapnel (TetraEnemyBullets.js) once it's been
 * alive for `seed.scatterDelay` seconds. A separate pool from the boss's own
 * TetraSeedBullets.js — several Tetra clones can be alive at once sharing
 * this ONE pool, at this type's own (weaker) Config.enemy.tetra.seed tuning
 * rather than Config.boss.tetra.seed's.
 *
 * Only ever launched by angle (`fireAngle` — one of the hull's 4 side
 * directions, see TetraEnemy._updateBounceFire), never aimed at the player.
 * Detonation is purely TIME-based, not "on arrival" — same reasoning as the
 * boss's own seed pool.
 *
 * Structurally a straight-line pooled bullet like EnemyBullet/TetraSeedBullets
 * (fixed velocity, bounds-culled, swap-remove-on-hit via `checkHit`), plus
 * one extra per-bullet `age` timer for the detonation clock.
 * `onDetonate(x, y)` is a fire-and-forget callback WaveManager wires in
 * `_buildProjectilePools` to fan the shrapnel pool out into an even ring —
 * this class only reports WHEN a burst happens, never what it looks like.
 *
 * Rendered individually (strokeCircle per bullet), with a growing radius as
 * `age` approaches `scatterDelay`, telegraphing the incoming burst.
 */
import { Config } from '../core/Config.js';

const MAX = Config.enemy.tetra.seed.poolSize;

export class TetraEnemySeedBullets {
  ...
  constructor({ onDetonate }) {
    ...
    const { color, lineWidth, glowBlur } = Config.enemy.tetra.seed;
  ...
  fireAngle(ox, oy, angle) {
    const { speed } = Config.enemy.tetra.seed;
  ...
  /** @param {number} dt */
  update(dt) {
    const { scatterDelay } = Config.enemy.tetra.seed;
  ...
  render(renderer) {
    ...
    const { scatterDelay, radius, growthMult } = Config.enemy.tetra.seed;
    for (let i = 0; i < this._count; i++) {
