...
export class SniperBullets {
   ...
   * for `accelDelay` seconds, then an ease-in ramp up to `maxSpeed` over
   * `accelDuration` seconds, then flat at `maxSpeed` forever after.
   * @param {number} age
   * @returns {number}
   */
  _speedAt(age) {
    ...
    return startSpeed + (maxSpeed - startSpeed) * easeInCubic(t);
  }
  ...
  update(dt) {
    ...
    for (let i = 0; i < this._count; i++) {
      this._age[i] += dt;
      const speed = this._speedAt(this._age[i]);
