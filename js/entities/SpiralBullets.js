 ...
 *
 * Structurally identical to EnemyBullet.js (fixed velocity, bounds-culled,
 * swap-remove-on-hit) — the only real differences are where its tuning
 * comes from (Config.boss.spiral.bullet instead of Config.enemyBullet) and
 * that it's fired by a raw angle (SpiralBoss aims along its own rotation,
 * not at the player), not a target point.
 */
...
export class SpiralBullets {
  ...
  render(renderer) {
    ...
    const hLen = Config.boss.spiral.bullet.halfLen;
    for (let i = 0; i < this._count; i++) {
