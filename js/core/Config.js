...
export const Config = Object.freeze({
  ...
  missionSelect: Object.freeze({
    ...
    lockedColor: "#6b7899", // dim — same "disabled" read as Shop's unaffordable color
  ...
  gameOver: Object.freeze({
    ...
    continue: Object.freeze({
      ...
      unaffordableColor: "#6b7899", // dim — reads as disabled, same as Shop's own unaffordable color
  ...
  screenShake: Object.freeze({
    ...
    reducedMotionScale: 0.25, // multiplies maxOffset when core/Settings.js's prefersReducedMotion() is true — scaled down, not zeroed, so a hit still reads as a hit
  }),
  ...
  damageFlash: Object.freeze({
    ...
    reducedMotionScale: 0.4, // multiplies peakAlpha under core/Settings.js's prefersReducedMotion() — see Config.screenShake.reducedMotionScale's own note
  }),
  ...
  dailyReward: Object.freeze({
    ...
    claimedBadge: Object.freeze({
      ...
      color: "#aab4d4",
      alpha: 0.75, // was 0.55 — composited over void that read ~3.5:1, below WCAG AA's 4.5:1 for text; 0.75 clears it (~5.8:1) while staying dimmer than the "TOMORROW" line's 0.8 below, preserving the two-tier hierarchy
      y: 884,
    ...
    streakStrip: Object.freeze({
      ...
      futureColor: "#6b7899", // matches Shop's buyUnaffordableColor / MissionSelect's lockedColor
  ...
  powerUps: Object.freeze({
    // Trimmed down from an initial 0.12 when the campaign was extended
    // from 10 to 30 levels — the back-half levels throw far more enemies
    // per wave, so the same fraction would otherwise mean a steadily
    // climbing ABSOLUTE pickup count as the campaign progresses, not a
    // steady one. The campaign has since grown again, to a full 50 levels
    // (see Config.waves.levels/Config.missionWaves.levels) without this
    // rate being revisited — worth re-checking pickup density at levels
    // 31-50 specifically before assuming 0.1 is still the right number.
    // `poolSize` below (8) is still the hard on-screen cap regardless of
    // this rate.
  ...
  combo: Object.freeze({
    ...
    // (.mp3, matching every other shipped SFX's format — the combo sting
    // will still need an actual audio asset recorded/placed here before
    // this feature is complete.)
    audioSrc: "assets/audio/streak.mp3",
    audioVolume: 0.7,
    audioPoolSize: 4,
  ...
  shop: Object.freeze({
    ...
    buyUnaffordableColor: "#6b7899", // dim — reads as "disabled," same intent as a grayed-out control
  ...
  settings: Object.freeze({
    ...
    toggle: Object.freeze({
      ...
      hitPaddingY: 18, // extra vertical hit-test margin, same convention/value as slider.hitPaddingY — the drawn 26vp height alone renders under the WCAG 24x24 CSS px touch-target floor at this game's real device scale
    }),
    segmented: Object.freeze({
      ...
      legSize: 8,
      activeColor: "#4DEFFF",
      inactiveColor: "#6b7899",
      font: '400 11px "Audiowide", "Courier New", monospace',
      hitPaddingY: 12, // see toggle.hitPaddingY's own note — same touch-target floor concern, smaller pad needed since the drawn height (30vp) starts closer to it
    }),
  ...
  playerSkill: Object.freeze({
    ...
    cooldownColor: "#9a7566", // lightened from #5a4038 (~2.1:1 over void, below WCAG AA) to ~4.9:1 — still a dim warm brown against the active #FF5C3D, just legible
